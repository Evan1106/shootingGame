...
public class MultiLoginScene_Server extends Scene {
    ...
    public void sceneBegin() {
        ...
        Client.getInstance().start(5300, "127.0.0.1");
        isOK = true;
...
//        Client.getInstance().start(5300, "192.168.1.28");
    ...
    public void update() {
        ...
        updateFrame(remotePlayer,PLAYER2);
        ...
        if(delayForChangeSence.count()){
            //要把剛剛角色的type傳到下一個場景
            MultiBattleMode_Server newMultiBattle = new MultiBattleMode_Server();
                    ...
                    case Global.Commands.UPDATE_FRAME:
                        if(command.getSerialNum() == PLAYER1 ) {
                            if (!command.getStrs().isEmpty()) {
                                localPlayer.setNewX(Integer.parseInt(command.getStrs().get(0)));
                                localPlayer.setNewY(Integer.parseInt(command.getStrs().get(1)));
                            }
                        }
                        break;
                    case Global.Commands.MAP:
                        if(command.getSerialNum() == PLAYER1){
                          selectedMap  = Integer.parseInt((command.getStrs().get(0)));
                        }
                        break;
                    default:
                        break;
...
//        //判斷主角有沒有撞牆
        if (commandCode == KeyEvent.VK_A) {
            remotePlayer.setLockRight(remotePlayer.collider().right() >= WINDOW_WIDTH);
        }
        if (commandCode == KeyEvent.VK_W) {
            remotePlayer.setLockBottom(remotePlayer.collider().bottom() >= WINDOW_HEIGHT - 50);
        }
        ...
        remotePlayer.keyPressed(commandCode,trigTime);
    ...
    public void updateFrame(GameObject o, int id) {
        ...
        Client.getInstance().sendCommand(new Client.Command(id, Global.Commands.UPDATE_FRAME,SERVER_IP,arr));
    }

    public void updateMap(int id){
        ArrayList<String> arr = new ArrayList<String>();
        arr.add(Integer.toString(randomMap));
        Client.getInstance().sendCommand(new Client.Command(id,Global.Commands.MAP,SERVER_IP,arr));
    }
}
