...
public class MultiBattleMode_Server extends Scene {
    ...
    @Override
    public void update() {
        updateFrame(hunter2,PLAYER2);
        updateTime(PLAYER2);
        updatePlayerHP(hunter2,PLAYER2);
        upfatePlayerKillAmount(PLAYER2);
                        ...
                        if (command.getSerialNum() == 1) {
                            ...
                            if (command.getStrs().get(0).equals("A") || command.getStrs().get(0).equals("a")) {
                                ...
                                if(hunter.getPlayerHP()>=0){
                                    hunter.changeState(Animator.State.WALK);
                    ...
                    case Global.Commands.UPDATE_FRAME:
                        if(command.getSerialNum() == PLAYER1 ) {
                            if (!command.getStrs().isEmpty()) {
                                hunter.setNewX(Integer.parseInt(command.getStrs().get(0)));
                                hunter.setNewY(Integer.parseInt(command.getStrs().get(1)));
                            }
                        }
                        break;
                    case Global.Commands.HP:
                        if(command.getSerialNum() == PLAYER1) {
                            hunter.setPlayerHP(Integer.parseInt(command.getStrs().get(0)));
                        }
                        break;
            ...
            if(!thisSceneisOver){
                if(passTime<TIME_LIMIT) {
                    ...
                    if(killAmount>againstKillAmount){
                        endingAnimator.setEndingEffect(EndingEffectAnimator.State.FINALE);
                        System.out.println("player2WIN");
                    }else if(againstKillAmount>killAmount){
                        endingAnimator.setEndingEffect(EndingEffectAnimator.State.FAIL);
                        System.out.println("player1WIN");
                    }else{
                        endingAnimator.setEndingEffect(EndingEffectAnimator.State.FINALE);
        ...
        /** 更新物件邏輯 */
        hpSingleDigit = hunter2.getPlayerHP()%10;
        hpTenDigit = hunter2.getPlayerHP()/10;


        //更新道具
        for (int i = 0; i < items.size(); i++) {
            Props item = items.get(i);
            if (item.isCollision(hunter2)) {
        ...
        for (int i = 0; i < ghosts.size(); i++) {
            ...
            if (ghost.isCollision(hunter2)) {
                if (ghost.state() != Monster.State.ELIMINATED) {
                    ...
                    hunter2.getHaunted();
                    hunter2.beAttacked(ghost.power());
                }
            }
            //怪物被地雷炸
            for (Props item : items) {
            ...
            //沒偵測到玩家 -> 原地左右移動
            if(!thisSceneisOver){
                if (ghost.isDetectPlayer(hunter2) || ghost.state() == Monster.State.TRACKING) {
    ...
    public void keyPressed(int commandCode, long trigTime) {
        ...
        if (commandCode == Global.D) {
            ...
            arr.add("D");
            Client.getInstance().sendCommand(new Client.Command(PLAYER2, Global.Commands.MOVE,SERVER_IP, arr));
        }
        ...
        hunter2.keyPressed(commandCode,trigTime);
    }

    @Override
    public void keyReleased(int commandCode, long trigTime) {
        hunter2.keyReleased(commandCode, trigTime);
    ...
    public void mouseTrig(MouseEvent e, CommandSolver.MouseState state, long trigTime){
        ...
        if (state == CommandSolver.MouseState.MOVED) {
            hunter2.facingDirection(e.getX() + camera.painter().left(), e.getY() + camera.painter().top());
        }
        ...
        if (state == CommandSolver.MouseState.CLICKED) {
            if (bulletAmount > 0) {
                bullets2.add(new Bullet(
                        ...
                        hunter2.painter().centerX(), hunter2.painter().centerY(), bulletSpeed, bulletPower));
    ...
    public void updateFrame(GameObject o, int id) {
        ...
        Client.getInstance().sendCommand(new Client.Command(id, Global.Commands.UPDATE_FRAME,"127.0.0.1",arr));
    }

    public void updateMonFrame(GameObject o, int id) {
        ArrayList<String> arr = new ArrayList<String>();
        arr.add(Integer.toString(o.painter().left()));
        arr.add(Integer.toString(o.painter().top()));
        Client.getInstance().sendCommand(new Client.Command(id, Global.Commands.UPDATE_FRAME,"127.0.0.1",arr));
    ...
    public void updateTime(int id){
        ...
        Client.getInstance().sendCommand(new Client.Command(id ,Global.Commands.TIME,"127.0.0.1",arr));

    }

//    public void updateMap(int id){
//        ArrayList<String> arr = new ArrayList<String>();
//        arr.add(Integer.toString(selectedMap));
//        Client.getInstance().sendCommand(new Client.Command(id,Global.Commands.MAP,"127.0.0.1",arr));
//    }

    public void updatePlayerHP(GameObject o, int id){
        ArrayList<String> arr = new ArrayList<String>();
        arr.add(Integer.toString(hunter2.getPlayerHP()));
        Client.getInstance().sendCommand(new Client.Command(id,Global.Commands.HP,"127.0.0.1",arr));
    }

    public void upfatePlayerKillAmount(int id){
        ArrayList<String> arr = new ArrayList<String>();
        arr.add(Integer.toString(killAmount));
        Client.getInstance().sendCommand(new Client.Command(id,Global.Commands.KILLAMOUNT,"127.0.0.1",arr));
...
//    }
    ...
    private void checkItem(Props item){
        if(hunter2.getPlayerHP()>0){
            hunter2.getPropsAnimator().setItemEffect(item.getCategory());
            hunter2.setUseItem();
            switch (item.getType()) {
                case PLAYER:
                    item.useOnPlayer(hunter2);
                    break;
                case SCENE:
    ...
    private void useOnScene(Props.Category category) {
        switch (category) {
            ...
            case FLARE:
                ...
                break;
            case LANDMINE:
                Props landMine = new Props(hunter2.painter().left(), hunter2.painter().top(), Props.Category.LANDMINE);
